...
in {
  ...
  config = lib.mkIf config.custom.hyprland.enable {
    home = {
      ...
      shellAliases = { hypr-log = "hyprctl rollinglog --follow"; };
    };
    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        monitor = (lib.forEach monitors (d:
        ...
        general = let gap = if host == "desktop" then 8 else 4;
        in {
          gaps_in = gap;
          gaps_out = gap;
          border_size = 2;
        ...
        lib.optionalAttrs (lib.length monitors > 1) {
          workspace = lib.custom.mapWorkspaces ({ workspace, monitor, ... }:
            "${workspace},monitor:${monitor.name}") monitors;
        };
    };

    # hyprland crash reports
    custom.persist = { home.cache.directories = [ ".cache/hyprland" ]; };
  };
}
